...
function SidebarContent() {
  ...
  useEffect(() => {
    ADMIN.getCountPendingList()
      .then((res) => {
        const countPendingTransAction = CHANGE_COUNT_PENDING_TRANS(parseInt(res.data.count));
    ...
    ADMIN.countRequestUser()
      .then((res) => {
        const countRequestUserAction = CHANGE_COUNT_REQUEST_USER(parseInt(res.data.count));
          ...
          route.routes ? (
            ...
            <li className="relative px-6 py-3" key={route.name_vn}>
              ...
              >
                ...
                <span className="ml-4">{i18n.language === "en" ? route.name_en : route.name_vn}</span>
                {
                  (route.path === "/admin/active" && countPendingTrans > 0) &&
                  <Badge type="danger" className="ml-2 font-semibold">{countPendingTrans <= 5 ? countPendingTrans : "5+"}</Badge>
                }
                {
                  (route.path === "/admin/request" && countRequestUser > 0) &&
                  <Badge type="danger" className="ml-2 font-semibold">{countRequestUser <= 5 ? countRequestUser : "5+"}</Badge>
                }
              </NavLink>
            </li>
          )
