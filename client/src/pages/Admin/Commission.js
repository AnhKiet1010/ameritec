...
function Receipts() {
  ...
  const DataSet = [
    {
      ...
      data: exportData.map((item) => {
        return [
          ...
          { value: PACKAGE.find(b => b.value === item.trans_info.buy_package).label },
          { value: (item.created_time && item.created_time !== "") ? new Date(item.created_time).toLocaleDateString('vi') : "" },
          { value: item.receive_mem_id },
          { value: item.trans_info.amount_vnd },
          { value: (item.trans_info.payment_method === "tienmat" ? "tiền mặt" : item.trans_info.payment_method === "nganluong" ? "Ngân Lượng" : item.trans_info.payment_method === "nganluongvisa" ? "Ngân Lượng Visa" : item.trans_info.payment_method === "PAYPAL" ? "PAYPAL" : item.trans_info.payment_method === "Credit Card" ? "Credit Card" : "") },
          { value: item.amount_vnd },
          { value: (item.approved_time && item.approved_time !== "") ? new Date(item.approved_time).toLocaleDateString('vi') : "" },
          { value: item.bank_account ? item.bank_account : "" },
          { value: item.bank ? item.bank : "" },
          { value: item.bank_name ? item.bank_name : "" },
          { value: item.status },
          { value: item.payment_method === "thucong" ? "tiền mặt" : item.payment_method === "nganluong" ? "Ngân Lượng" : "" },
  ...
  useEffect(() => {
    ...
    ADMIN.getReceipts({ currentTable, page, resultsPerPage, keyword, currentYear, currentMonth, searchLevel, searchType, changeListExport })
      .then((res) => {
  ...
  return (
    <>
      ...
      <div className="flex justify-center items-center mb-4">
        <div onClick={() => handleCurrentTableChange("pending")} className={`border-blue-500 transition-all cursor-pointer ${currentTable === 'pending' && "border-b-2 font-bold"} uppercase px-4 py-2`}>hoa hồng chờ duyệt</div>
        <div onClick={() => handleCurrentTableChange("processing")} className={`border-blue-500 transition-all cursor-pointer ${currentTable === 'processing' && "border-b-2 font-bold"} uppercase px-4 py-2`}>hoa hồng đang xử lí</div>
        <div onClick={() => handleCurrentTableChange("success")} className={`border-blue-500 transition-all cursor-pointer ${currentTable === 'success' && "border-b-2 font-bold"} uppercase px-4 py-2`}>hoa hồng đã duyệt</div>
      </div>
            ...
            {(searchType == '3') &&
              <div className="relative">
                <select
                  ...
                  <option value={1} defaultValue={searchLevel === 1}>Gói cá nhân</option>
                  <option value={2} defaultValue={searchLevel === 2}>Gói khởi nghiệp</option>
                  <option value={3} defaultValue={searchLevel === 3}>Gói doanh nghiệp A</option>
                  <option value={4} defaultValue={searchLevel === 4}>Gói doanh nghiệp B</option>
                </select>
                <div
                  className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                  <svg className="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
        ...
        <Table>
          <TableHeader>
            <tr>
              ...
              <TableCell className="text-center">Hình thức thanh toán</TableCell>
              ...
              <TableCell className="text-center">Trạng thái</TableCell>
              {
                ...
                <TableCell className="text-center">Phương thức trả HH</TableCell>
              }
                ...
                <>
                  <TableRow key={i}>
                    ...
                    <TableCell className="text-center">
                      <div className={`min-w-max px-4 py-2 text-center rounded-md bg-${com.trans_info.buy_package === '1' ? 'gray-600' : com.trans_info.buy_package === '2' ? 'green-500' : com.trans_info.buy_package === '3' ? "blue-500" : com.trans_info.buy_package === '4' ? 'cyan-default' : "black"} text-white text-sm`}>
                        {
                          com.trans_info.account_type === 'en' ? PACKAGE.find(ele => ele.value === com.trans_info.buy_package).label_en
                            : PACKAGE.find(ele => ele.value === com.trans_info.buy_package).label
                        }
                      </div>
                    </TableCell>
                    <TableCell className="text-center">
                    ...
                    <TableCell className="text-center">
                      {com.trans_info.payment_method === 'tienmat' && 'Tiền mặt'}
                      {com.trans_info.payment_method === 'nganluong' && 'Ngân Lượng ATM'}
                      {com.trans_info.payment_method === 'nganluongvisa' && 'Ngân Lượng VISA'}
                      {com.trans_info.payment_method === 'PAYPAL' && 'PAYPAL'}
                      {com.trans_info.payment_method === "Credit Card" && "Credit Card"}
                    </TableCell>
                    <TableCell className="text-center">
                    ...
                    <TableCell className="text-center">
                      {
                        com.account_type === 'en' ?
                          <span className="text-sm">{com.amount_usd.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,")} USD</span>
                          :
                          <span className="text-sm">{com.amount_vnd.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,")} VNĐ</span>
                      }
                    </TableCell>
                    <TableCell className="text-center">
                      {
                        com.is_renew ? <Badge type="warning">Gia hạn</Badge> : <Badge type="neutral">Đăng ký mới</Badge>
                      }
                    </TableCell>
                    {
                      currentTable === "success" &&
                    ...
                    <TableCell>
                      <div className="flex m-auto justify-center py-4">
                        <a href={`/admin/commission/${com._id}`}
