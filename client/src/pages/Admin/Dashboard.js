...
import { CartIcon, MoneyIcon, PeopleIcon, FormsIcon, OutlineLogoutIcon } from "../../icons";
  ...
  const DataSet = [
    {
      ...
      data: exportData ? exportData.map((item) => {
        let gender = "N/A";
      ...
      datasets: [
        {
           ...
           */
          backgroundColor: ['#10B981', '#3B82F6', '#F97316', '#F59E0B', '#EF4444', '#8B5CF6', '#EC4899'],
    ...
    ADMIN.rank()
      .then((res) => {
        ...
        if (status !== 200) {
          ...
          setData(res.data.data.listUser.slice((page - 1) * resultsPerPage, page * resultsPerPage))
                ...
                <TableFooter>
                  {data.length !== 0 ?
                    <div className="flex justify-between items-center">
                      ...
                      <div>Có <span className="text-xl mx-1 text-gray-700">{500}</span> kết quả</div>
                    </div>
                    :
          ...
          <TableContainer className="">
            <Table>
              <TableHeader className="">
                <tr>
                  <TableCell>Họ và tên</TableCell>
                  <TableCell className="text-center">Đã hoàn thành bước</TableCell>
                  <TableCell className="text-center">Điểm</TableCell>
                </tr>
              </TableHeader>
              {!loadingTable &&
