...
function Dashboard() {
  ...
  const [currentTable, setCurrentTable] = useState(role === 'admin' ? 'pending' : 'success');
    ...
    ADMIN.activeTrans(id)
      .then((res) => {
        ...
        if (status !== 200) {
          ...
          const countPendingTransAction = CHANGE_COUNT_PENDING_TRANS(res.data.data.count);
  ...
  const DataSet = [
    {
      ...
      data: exportData.map((item) => {
        return [
          ...
          { value: (item.payment_method === "tienmat" ? "tiền mặt" : item.payment_method === "nganluong" ? "Ngân Lượng" : item.payment_method === "nganluongvisa" ? "Ngân Lượng Visa" : "") },
  ...
  useEffect(() => {
    ...
    ADMIN.getPendingList({ keyword, page, resultsPerPage, searchType, searchLevel, currentTable, changeListExport })
      .then((res) => {
        const status = res.data.status;
        if (status !== 200) {
  ...
  return (
    <>
      ...
      <div className="flex justify-center items-center mb-4">
        {
          role === 'admin' &&
          <div onClick={handleCurrentTableChange} className={`border-blue-500 transition-all cursor-pointer ${currentTable === 'pending' && "border-b-2 font-bold"} uppercase px-4 py-2`}>giao dịch chờ </div>
        }
        <div onClick={handleCurrentTableChange} className={`border-blue-500 transition-all cursor-pointer ${currentTable === 'success' && "border-b-2 font-bold"} uppercase px-4 py-2`}>giao dịch thành công</div>
      </div>
            ...
            <div className="flex flex-row mb-1 sm:mb-0">
              ...
              <div className="relative">
                <select
                  ...
                  <option value={2} defaultValue={searchType === 2}>Theo Gói</option>
                </select>
                <div
                  className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                  <svg className="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
      ...
      <TableContainer className="mt-4">
        <Table>
          <TableHeader className="bg-gray-300">
            {
              ...
              <tr>
                ...
                <TableCell className="text-center">Phương thức thanh toán</TableCell>
            ...
            }
            {
              ...
              <tr>
                ...
                <TableCell className="text-center">Người giới thiệu</TableCell>
                {/* <TableCell className="text-center">Số điện thoại</TableCell> */}
                <TableCell className="text-center">Thời gian thanh toán</TableCell>
                <TableCell className="text-center">Phương thức thanh toán</TableCell>
                <TableCell className="text-center">Gói</TableCell>
                <TableCell className="text-center">Trạng thái</TableCell>
          ...
          </TableHeader>
          {
            !loading &&
            <TableBody>
              {currentTable === "pending" && data.map((tran, i) => (
                <TableRow key={i} className={`${i % 2 !== 0 && 'bg-gray-100'}`}>
                  <TableCell>
                    <div className="flex items-center text-sm">
                      <div>
                        <p className="font-semibold">{tran.user_name}</p>
                        <p className="text-xs text-gray-600 dark:text-gray-400">{tran.email}</p>
                      </div>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="text-center">
                    <span className="text-sm">
                      {`${new Date(tran.created_time).toLocaleTimeString('vi')} - 
                  ...
                  <TableCell className="text-center">
                    ...
                    {tran.payment_method === 'nganluongvisa' && 'Ngân Lượng VISA'}
                    {tran.payment_method === 'PAYPAL' && 'PAYPAL'}
                    {tran.payment_method === "Credit Card" && "Credit Card"}
                  </TableCell>
                  <TableCell className="text-center">
                    <div className={`min-w-max px-4 py-2 text-center rounded-md bg-${tran.buy_package === '1' ? 'gray-600' : tran.buy_package === '2' ? 'green-500' : tran.buy_package === '3' ? 'blue-700' : "cyan-default"} text-white text-sm`}>
                      {
                        tran.account_type === 'en' ? PACKAGE.find(ele => ele.value === tran.buy_package).label_en
                          : PACKAGE.find(ele => ele.value === tran.buy_package).label
                      }
                    </div>
                  </TableCell>
                  <TableCell className="text-center">
                    ...
                    {
                      tran.is_renew ? <Badge type="warning">Gia hạn</Badge> : <Badge type="neutral">Đăng ký mới</Badge>
                    }
                  </TableCell>
              ...
              {currentTable === "success" && data.map((tran, i) => (
                <TableRow key={i} className={`${i % 2 !== 0 && 'bg-gray-100'}`}>
                  <TableCell>
                    <div className="flex items-center text-sm">
                      <div>
                  ...
                  <TableCell className="text-center">
                    <span className="text-sm">
                      {`${new Date(tran.approved_time).toLocaleTimeString('vi')} - 
                  ...
                  <TableCell className="text-center">{tran.approved_by}</TableCell>
