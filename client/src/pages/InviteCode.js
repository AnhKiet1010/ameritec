...
function InviteCode() {
  ...
  useEffect(() => {
    ...
    const body = { id: JSON.parse(localStorage.getItem("user")).user.id, page, resultsPerPage, keyword };
  ...
  useEffect(() => {
    setUrl(
      `${process.env.REACT_APP_REFERRAL_URL}/${inviteCode}/${group}/${donate ? donateID : ""
      }`
    );
      ...
      <TableContainer className="mb-8">
        {
          !loading && <Table>
            <TableBody>
              <TableRow>
                <TableCell className="text-md font-semibold">{t('Mã giới thiệu')}</TableCell>
                <TableCell>
              ...
              {
                (buy_package === "2" || buy_package === "3" || buy_package === "4") &&
                <>
                  <TableRow>
                    <TableCell className="text-md font-semibold">{t('Chọn nhóm để gắn cây hệ thống')}</TableCell>
                    <TableCell>
                      <div className="grid md:grid-cols-3 md:grid-cols-2">
                        <Label check>
                          <Input className="border border-gray-700" type="radio" name="group" value={1} onChange={(e) => {
                            setGroup(e.target.value);
                          }} checked={group == 1} />
                          <span className="m-0 ml-2">{t('Nhóm 1')}</span>
                        </Label>
                        <Label check>
                          <Input className="border border-gray-700" type="radio" name="group" value={2} onChange={(e) => {
                            setGroup(e.target.value);
                          }} />
                          <span className="m-0 ml-2">{t('Nhóm 2')}</span>
                        </Label>
                        <Label check>
                          <Input className="border border-gray-700" type="radio" name="group" value={3} onChange={(e) => {
                            setGroup(e.target.value);
                          }} />
                  ...
                  <TableRow>
                    <TableCell className="text-md font-semibold">{t('Cho doanh số cấp dưới')}</TableCell>
                    <TableCell>
                      <div className="grid grid-cols-2">
                        <Label check>
                          <Input className="border border-gray-700" type="radio" name="donate" value={1} onChange={(e) => {
                            setDonate(true);
                          }} />
                          <span className="ml-2 mb-0">{t('Có')}</span>
                        </Label>
                        <Label check>
                          <Input className="border border-gray-700" type="radio" name="donate" value={2} onChange={(e) => {
                            setDonate(false);
                            setDonateID("");
                          }} />
                  ...
                  {donate && (
                    <TableRow>
                      <TableCell colSpan={2}>
                        <div className="mb-4 text-md font-semibold">{t('Chọn ID được hưởng doanh số')}</div>
