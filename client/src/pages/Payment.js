...
import { Button, Label, Input, Modal, ModalHeader, ModalBody } from "@windmill/react-ui";
...
function Payment({ match }) {
  ...
  useEffect(() => {
    ...
    API.checkTransId({ transId })
      .then((res) => {
        ...
        if (status === 200) {
          let { user_name, phone, amount_usd, amount_vnd, buy_package, email } = res.data.data.trans;
          setName(user_name);
          setPhone(phone);
          setEmail(email);
          setBuyPackage(buy_package);
          setTotalAmount(i18n.language.includes('vi') ? amount_vnd : amount_usd);
        } else {
          history.push("/");
        }
      }).catch((err) => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    bank_code: isNLATM ? Yup.string().required("Vui lòng chọn ngân hàng") : Yup.string().nullable(),
  });
    ...
    API.transaction(body)
      .then((res) => {
        ...
        if (status && status !== 200) {
          ...
          } else if (res.data.data.payment_method === "nganluong" || res.data.data.payment_method === "nganluongvisa") {
            actions.setSubmitting(false);
  ...
  return (
    <div className="flex min-h-screen md:p-6 bg-gray-50 ">
      <Modal isOpen={openMoneyChase}>
        <ModalHeader className="text-center mb-4">🎉 {t('Thông Báo')} 🎉</ModalHeader>
        <ModalBody>
          <div className="text-lg md:text-xl flex flex-col justify-center">
            {
              i18n.language === "vi" &&
              <p>
                Giao dịch của bạn đã vào trạng thái chờ! Vui lòng thanh toán để được kích hoạt. Bạn có thể thanh toán tại Văn Phòng Công Ty hoặc chuyển khoản qua số tài khoản: 118002775135, ngân hàng: VietinBank, Tên tài khoản: CÔNG TY CỔ PHẦN AMERITEC
              </p>
            }
            <br />
            <p className="text-center">
              {t('Tài khoản sau khi xác nhận xong sẽ được thông báo về Email mà bạn đã đăng ký!')}
            </p>
      ...
      <Modal isOpen={openChangePage}>
        <ModalBody>
          <div className="flex flex-col justify-center">
            <Button>
              <a className="text-white" href={url}>Thanh toán</a>
            </Button>
          </div>
        </ModalBody>
      </Modal>
        ...
        <main className="flex p-6">
          <div className="w-full">
                ...
                return (
                  <Form className="flex justify-center flex-col">
                    <div className="main w-full mb-4">
                      <div className="flex items-center w-full">
                        <div className="rounded-md mr-6">
                          <img src={package_img} alt="package" width="100px" className="object-fit rounded-md" />
                        </div>
                        {buy_package === "1" && <h5 className="font-semibold">{t('Gói Cá Nhân')}</h5>}
                        {buy_package === "2" && <h5 className="font-semibold">{t('Gói Khởi Nghiệp')}</h5>}
                        {buy_package === "3" && <h5 className="font-semibold">{t('Gói Doanh Nghiệp')}</h5>}
                      </div>
                    </div>
                    <div className="mt-3 w-full"><label className="flex text-md text-gray-700 dark:text-gray-400">
                    ...
                    <div className="mt-3 w-full"><label className="flex text-md text-gray-700 dark:text-gray-400" >
                      ...
                      <span className="font-semibold">{total_amount.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,")} {i18n.language.includes('vi') ? 'VNĐ' : 'USD'}</span>
                    </label></div>
                    ...
                    <FastField
                      ...
                      options={[{ value: "VN", label: "Việt Nam" }, { value: "USA", label: "The United States of America" }]}
                      label={t('Quốc gia')}
                    />
                    {
                      isNLATM && <FastField
                    ...
                    <div className="grouped fields mt-4">
                      ...
                      <div className="flex flex-col mt-3">
                        <Label radio className="flex items-center mt-2 text-md">
                          <Input type="radio" className="border border-gray-700" name="payment_method" value="tienmat" onChange={(e) => {
                        ...
                        </Label>
                        {
                          ...
                          <>
                            <Label radio className="flex items-center mt-2 text-md">
                              <Input type="radio" className="border border-gray-700" name="payment_method" value="nganluong" onChange={(e) => {
                                setIsChooseMethod(true);
                                setShowErrChooseMethod(false);
                                setFieldValue("payment_method", e.target.value);
                                setIsNLATM(!isNLATM);
                            ...
                            <Label radio className="flex items-center mt-2">
                              <Input type="radio" className="border border-gray-700" name="payment_method" value="nganluongvisa" onChange={(e) => {
                                setIsChooseMethod(true);
                                setFieldValue("payment_method", e.target.value);
                                setIsNLATM(false);
                              }} />
                              <span className="m-0 ml-2 text-md">Ngân Lượng (VISA)</span>
                            </Label>
                          </>
                        }
                      </div>
                      {
                        showErrChooseMethod &&
                        <div className="mt-4 text-red-500">Vui lòng chọn phương thức thanh toán</div>
                      }
                    </div>
                    <br />
                    {/* {
...

            {
              !i18n.language.includes('vi') &&
              <div className="my-6">
                <Button size="large" onClick={() => setOnCredit(true)}>Pay with Credit card</Button>
                ...
                <Modal isOpen={onCredit} onClose={onCancelCredit}>
                  <ModalHeader>
                    <p className="text-center mb-4 text-lg font-semibold">Pay with Credit Card</p>
                  </ModalHeader>
                  <ModalBody>
                    <Formik
                      ...
                      {(formikProps) => {
                        ...
                        return (
                          <Form>
                            ...
                            {
                              isSubmitting &&
                              <p text="sm text-red my-2 mx-auto">Please don't click Close button</p>
                            }
                            <div className="flex justify-center">
                              <Button type="submit" className="mt-2 m-auto" disabled={isSubmitting}>
                                {isSubmitting ? (
                                  <Spinner />
                                ) : (
                                  <i className="fas fa-sign-in-alt  w-6 -ml-2 mx-3" />
            ...
            }
            {
              ...
              <div className="my-6">
                {checkout ? (
                  ...
                  <div>
                    <Button size="large" className="appearance-none"
                      onClick={() => {
                        setCheckOut(true);
                      }}
                    >
                    ...
                    <div className="mt-2">
                      ...
                      <p className="font-bold mt-2 text-sm">Pay by PayPal, additional fees 4.59%</p>
                    </div>
                  </div>
                )}
              </div>
