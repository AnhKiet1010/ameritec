...
function Register({ match }) {
  ...
  if (buy_package !== '1' && buy_package !== '2' && buy_package !== '3' && buy_package !== '4') {
    return <Redirect to="/" />
  }
  const currentPackage = lang.includes('vi') ? PACKAGE.find((ele) => buy_package === ele.value).label : PACKAGE.find((ele) => buy_package === ele.value).label_en;
  ...
  const validationSchema23 = Yup.object().shape({
    ...
    phone: Yup.string().required(t("Vui lòng điền số điện thoại")).matches(phoneRegExp, t('Số điện thoại không đúng định dạng')),
    birthday: Yup.string()
    ...
    id_code: lang.includes('vi') ? Yup.string().required(t("Vui lòng điền Số CMND")) : Yup.string().nullable(),
    id_time: lang.includes('vi') ? Yup.string().required(t("Vui lòng Chọn Ngày Cấp")) : Yup.string().nullable(),
    gender: lang.includes('vi') ? Yup.string().required(t("Vui lòng điền giới tính")) : Yup.string().nullable(),
    issued_by: lang.includes('vi') ? Yup.string().required(t("Vui lòng Chọn Nơi Cấp")) : Yup.string().nullable(),
    bank: lang.includes('vi') ? Yup.string().required(t("Vui lòng Chọn tên Ngân Hàng")) : Yup.string().nullable(),
    bank_account: lang.includes('vi') ? Yup.string().required(t("Vui lòng điền Số Tài Khoản")) : Yup.string().nullable(),
    bank_name: lang.includes('vi') ? Yup.string().required(t("Vui lòng điền Tên Tài Khoản")) : Yup.string().nullable(),
    // CMND_Front: Yup.string().required(t("Vui lòng tải lên hình ảnh")),
    // CMND_Back: Yup.string().required(t("Vui lòng tải lên hình ảnh")),
    state: !lang.includes('vi') ? Yup.string().required("Please input state") : Yup.string().nullable(),
    ss: !lang.includes('vi') ? Yup.string().required("Please input ID code") : Yup.string().nullable(),
    request_commission: !lang.includes('vi') ? Yup.string().required("Please input Froms of receiving commissions") : Yup.string().nullable(),
    drive_id: !lang.includes('vi') ? Yup.string().required("Please input Driver's License") : Yup.string().nullable(),
  });
  ...
  const handleSubmit = (values, actions) => {
    ...
    if (!values.accept_confirm) {
      actions.setFieldError("accept_confirm", t("Vui lòng đồng ý với điều khoản Công Ty"));
      actions.setSubmitting(false);
    } else if (isPartner && lang.includes('vi') && (imgUrlMT === "" || imgUrlMS === "")) {
      if (imgUrlMT === "") {
      ...
      const body = (isPartner && lang.includes('vi')) ? formData : { account_type: lang, is_partner: isPartner, ...values };
        ...
        <div className="flex-1 h-full max-w-3xl">
          <div className="flex flex-col overflow-y-auto">
            <Formik
              ...
              validationSchema={!isPartner ? validationSchema1 : validationSchema23}
                  ...
                  <Form className="flex items-center justify-center p-6 sm:p-12" encType="multipart/form-data">
                    <div className="w-full">
                      <div className="flex items-center justify-between">
                        ...
                        <a href="/"><img src={LOGO} width="100px" alt="logo" /></a>
                      </div>
                      <hr className="my-6" />
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-4 mt-4">
                        <Label check>
                          <Input type="checkbox" checked={isPartner} className="text-purple-600 border border-gray-700 form-checkbox focus:border-purple-400 focus:outline-none focus:shadow-outline-purple" onChange={() => setIsPartner(true)} />
                          <span className="ml-2 mb-0 text-md text-gray-700">{t('Đăng ký làm Cộng Tác Viên')}</span>
                        </Label>
                        <Label check>
                          <Input type="checkbox" checked={!isPartner} className="text-purple-600 border border-gray-700 form-checkbox focus:border-purple-400 focus:outline-none focus:shadow-outline-purple" onChange={() => setIsPartner(false)} />
                          <span className="ml-2 mb-0 text-md text-gray-700">{t('Mua để sử dụng')}</span>
                        </Label>
                      </div>
                      {
                        isPartner && <>
                          ...
                          }
                          {
                            ...
                            <>
                              ...
                              </div>
                              {
                                ...
                                <div className="w-full text-sm mt-2 error invalid-feedback text-center">{t('Vui lòng tải lên hình ảnh')}</div>
                              }
                              <div className="my-2">
