...
const checkLevel = async (id) => {
  ...
  console.log("sumPoint", sumPointGroup1 + "/" + sumPointGroup2 + "/" + sumPointGroup3);
  ...
  var count_check1 = await User.countDocuments({ $and: [{ _id: { $in: tree.group1 } }, { buy_package: { $ne: "1" } }, { expired: false }] }).exec();
  var count_check2 = await User.countDocuments({ $and: [{ _id: { $in: tree.group2 } }, { buy_package: { $ne: "1" } }, { expired: false }] }).exec();
