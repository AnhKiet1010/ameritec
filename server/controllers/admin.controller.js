...
exports.getDashboard = async (req, res) => {
  const countPersonPackages = await User.countDocuments({ buy_package: "1" }).exec();
  const countStartupPackages = await User.countDocuments({ buy_package: "2" }).exec();
  const countBusinessPackages = await User.countDocuments({ $and: [{ buy_package: "3" }, { is_clone: { $ne: true } }] }).exec();
  const countBusinessPackagesB = await User.countDocuments({ $and: [{ buy_package: "4" }, { is_clone: { $ne: true } }] }).exec();
...
exports.getRank = async (req, res) => {
  const listUser = await User.find({ role: 'normal', is_delete: false, active: true, expired: false }).limit(500).sort({ level: -1, point: -1, _id: -1 }).exec();
  res.json({
    status: 200,
    data: {
      listUser,
...
exports.getUsers = async (req, res) => {
  ...
  const currentYear = req.body.currentYear === 'all' ? 'all' : parseInt(req.body.currentYear);
  ...
  if (!getExportData) {
    if (searchType === 1) {
      listUser = await User.find({ $and: [{ role: "normal" }, { is_delete: false }] }).sort({ created_time: -1, _id: -1 }).limit(perPage).skip(skip).exec();
      countAllDocument = await User.countDocuments({ $and: [{ role: "normal" }, { is_delete: false }] }).sort({ created_time: -1, _id: -1 }).exec();
    }

    if (searchType === 2) {
      listUser = await User.find({ $and: [{ role: "normal" }, { uname: { $regex: '.*' + removeAccents(keyword) + '.*', $options: 'i' } }, { is_delete: false }] }).limit(perPage).skip(skip).sort({ created_time: -1, _id: -1 }).exec();
      countAllDocument = await User.countDocuments({ $and: [{ role: "normal" }, { uname: { $regex: '.*' + keyword + '.*', $options: 'i' } }, { is_delete: false }] }).sort({ created_time: -1, _id: -1 }).exec();
    }

    if (searchType === 3) {
      listUser = await User.find({ $and: [{ role: "normal" }, { email: { $regex: '.*' + keyword + '.*', $options: 'i' } }, { is_delete: false }] }).limit(perPage).skip(skip).sort({ created_time: -1, _id: -1 }).exec();
      countAllDocument = await User.countDocuments({ $and: [{ role: "normal" }, { email: { $regex: '.*' + keyword + '.*', $options: 'i' } }, { is_delete: false }] }).sort({ created_time: -1, _id: -1 }).exec();
    }

    if (searchType === 4) {
      if (keyword.length === 24) {

        listUser = await User.find({ $and: [{ role: "normal" }, { _id: keyword }, { is_delete: false }] }).sort({ created_time: -1, _id: -1 }).limit(perPage).skip(skip).exec();
        countAllDocument = await User.countDocuments({ $and: [{ role: "normal" }, { _id: keyword }, { is_delete: false }] }).sort({ created_time: -1, _id: -1 }).exec();
      } else {
        listUser = [];
        countAllDocument = 0;
      }
    ...
    if (searchType === 5) {
      listUser = await User.find({ $and: [{ role: "normal" }, { level: searchLevel }, { is_delete: false }] }).sort({ created_time: -1, _id: -1 }).limit(perPage).skip(skip).exec();
      countAllDocument = await User.countDocuments({ $and: [{ role: "normal" }, { level: searchLevel }, { is_delete: false }] }).sort({ created_time: -1, _id: -1 }).exec();
    }
    ...
    if (searchType === 6) {
      ...
      }).sort({ created_time: -1, _id: -1 }).limit(perPage).skip(skip).exec();
      countAllDocument = await User.countDocuments({
        ...
        { is_delete: false }]
      }).sort({ created_time: -1, _id: -1 }).exec();
    }
    ...
    if (searchType === 7) {
      if (currentYear === 'all') {
        ...
        let firstDay = new Date(currentYear - 1, currentMonth - 1, 1).toISOString();
    ...
    for (let user of listUser) {
      let parent_name = "";
      if (user.parent_id && user.parent_id !== "" && user.parent_id !== process.env.INVITE_CODE) {
        let parent = await User.findOne({ _id: user.parent_id, is_delete: false }).exec();
        if (parent) {
  ...
  if (getExportData) {
    if (searchType === 1) {
      exportData = await User.find({ $and: [{ role: "normal" }, { is_delete: false }] }).sort({ created_time: -1, _id: -1 }).exec();
    }

    if (searchType === 2) {
      exportData = await User.find({ $and: [{ role: "normal" }, { uname: { $regex: '.*' + keyword + '.*', $options: 'i' } }, { is_delete: false }] }).sort({ created_time: -1, _id: -1 }).exec();
    }

    if (searchType === 3) {
      exportData = await User.find({ $and: [{ role: "normal" }, { email: { $regex: '.*' + keyword + '.*', $options: 'i' } }, { is_delete: false }] }).sort({ created_time: -1, _id: -1 }).exec();
    }

    if (searchType === 4) {
      exportData = await User.find({ $and: [{ role: "normal" }, { old_id: keyword }, { is_delete: false }] }).sort({ created_time: -1, _id: -1 }).exec();
    }

    if (searchType === 5) {
      exportData = await User.find({ $and: [{ role: "normal" }, { level: searchLevel }, { is_delete: false }] }).sort({ created_time: -1, _id: -1 }).exec();
    }
...
exports.createUser = async (req, res) => {
  ...
  if (buy_package === "" || buy_package === null) {
    errors.push({ label: "buy_package", err_message: "Gói tham gia không được để trống" });
  }
  if (parent_id !== process.env.INVITE_CODE) {
    const repeat_parent_id = await User.find({ _id: parent_id, is_delete: false }).exec();
    if (repeat_parent_id.length < 0) {
      errors.push({ label: "parent_id", err_message: "Mã giới thiệu không đúng" });
    }
  }

  const user_repeat_id_code = await User.findOne({ $and: [{ id_code: id_code }, { id_code: { $ne: "" } }, { is_delete: false }] }).exec();
    ...
    bcrypt.genSalt(saltRounds, async function (err, salt) {
      bcrypt.hash(password, salt, async function (err, hash) {
        ...
        const user = new User({
          ...
          expire_time: new Date(datenow.getFullYear() + 1, datenow.getMonth(), datenow.getDate()),
          phone,
          group_number,
          id_code,
          id_time,
...
exports.getUser = async (req, res) => {
  ...
  if (listOldEmail.length > 0) {
    oldEmailObj = listOldEmail.map((ele, index) => {
      return { label: `Email cũ ${index + 1}`, value: `${ele.email} - ${new Date(ele.time_change).toLocaleString("vi", {timeZone: "Asia/Ho_Chi_Minh"})}` }
    });
  }
  var listLink = [];
  ...
  if (user.buy_package === "1") {
    listLink = [
      { label: "Link giới thiệu", value: `${process.env.CLIENT_URL}/referral/${user._id}` },
    ];
  }
  ...
  if (user.account_type === 'vi') {
    ...
    listInfo = [
      ...
      { label: "Froms of receiving commissions", value: user.request_commission },
    ]
  }
  ...
  res.json({
    ...
    data: {
      ...
      result: [
        ...
        { label: "Trạng thái", value: user.expired === true ? "Đã hết hạn" : "Đang hoạt động" },
        { label: "Ngày tháng năm sinh", value: user.birthday ? new Date(user.birthday).toLocaleDateString("vi").split(",")[0] : "" },
        { label: "Ngày tạo tài khoản", value: user.created_time ? new Date(user.created_time).toLocaleDateString("vi").split(",")[0] : "" },
        { label: "Ngày hết hạn tài khoản", value: user.expire_time ? new Date(user.expire_time).toLocaleDateString("vi").split(",")[0] : "" },
        ...
        ...listInfo,
        ...listLink,
        { label: "Phân loại tài khoản", value: `${user.is_partner ? 'Cộng Tác Viên' : 'Mua để dùng'} / ${user.account_type === 'en' ? 'Nước ngoài' : 'Trong nước'}` },
...
exports.editUser = async (req, res) => {
  ...
  if (email !== old_email) {
    const valid_email = await User.findOne({ $and: [{ email }, { _id: { $ne: id } }, { is_clone: false }, { is_delete: false }] }).exec();
    if (valid_email) {
  ...
  if (errors.length > 0) {
    ...
    if (user.full_name !== full_name) {
      for (var a of arr_id_clone) {
        var clone_acc = await User.findOne({ _id: mongoose.Types.ObjectId(a) }).exec();
        ...
        await Transaction.updateMany({ user_id: a }, { user_name: clone_acc.full_name, user_uname: clone_acc.uname }).exec();
        await Transaction.updateMany({ invite_id: a }, { invite_name: clone_acc.full_name, invite_uname: clone_acc.uname }).exec();
        await Commission.updateMany({ join_mem_id: a }, { join_mem_name: clone_acc.full_name, join_mem_uname: clone_acc.uname }).exec();
        await Commission.updateMany({ receive_mem_id: a }, { receive_mem_name: clone_acc.full_name, receive_mem_uname: clone_acc.uname }).exec();
        await Bonus.updateMany({ receive_mem_id: a }, { receive_mem_name: clone_acc.full_name }).exec();
      }
      ...
      //   }
      // ).exec();
      await Transaction.updateMany({ user_id: id }, { user_name: full_name, user_uname: await removeAccents(full_name) }).exec();
      await Transaction.updateMany({ invite_id: id }, { invite_name: full_name, invite_uname: await removeAccents(full_name) }).exec();
      await Commission.updateMany({ join_mem_id: id }, { join_mem_name: full_name, join_mem_uname: await removeAccents(full_name) }).exec();
      await Commission.updateMany({ receive_mem_id: id }, { receive_mem_name: full_name, receive_mem_uname: await removeAccents(full_name) }).exec();
      await Bonus.updateMany({ receive_mem_id: id }, { receive_mem_name: full_name }).exec();
      change = true;
    }
    ...
    if (user.email !== email) {
      ...
      var array = [];
      array = [...user.old_email, { email: old_email, time_change: new Date() }];
      await User.findOneAndUpdate(
        { _id: id },
        {
          email,
    ...
    if (user.expire_time !== expire_time) {
      ...
      await Transaction.findOneAndUpdate({ user_id: id }, { expired_time: expire_time }).exec();
      change = true;
    }
    if (user.buy_package !== buy_package) {
      ...
      await Transaction.findOneAndUpdate({ user_id: id }, { buy_package }).exec();
      change = true;
    }
    //check password
    if (password) {
...
exports.getStorage = async (req, res) => {
  ...
  if (searchType === 1) {
    listLinkFilter = await Activation.find({}).sort({ _id: -1 }).limit(perPage).skip(skip).exec();
    countAllDocument = await Activation.countDocuments({}).sort({ _id: -1 }).exec();
  }

  if (searchType === 2) {
    listLinkFilter = await Activation.find({ firstName: { $regex: '.*' + keyword + '.*', $options: 'i' } }).limit(perPage).skip(skip).sort({ _id: -1 }).exec();
    countAllDocument = await Activation.countDocuments({ firstName: { $regex: '.*' + keyword + '.*', $options: 'i' } }).sort({ _id: -1 }).exec();
  }
  ...
  if (changeExportList) {
    ...
    if (searchType === 2) {
      exportData = await Activation.find({ firstName: { $regex: '.*' + keyword + '.*', $options: 'i' } }).sort({ _id: -1 }).exec();
    }
...
exports.getTree = async (req, res) => {
  ...
  if (searchType === 1) {
    countAllDocument = await User.countDocuments({ $and: [{ role: 'normal' }, { parent_id: process.env.INVITE_CODE }, { is_delete: false }] }).exec();
    listAgency = [... (await User.find({ $and: [{ role: 'normal' }, { parent_id: process.env.INVITE_CODE }, { is_delete: false }] }).skip(skip).limit(perPage)
      .sort({
        _id: -1
      }).exec())];
  }
  ...
  if (searchType === 2) {
    countAllDocument = await User.countDocuments({ $and: [{ role: 'normal' }, { uname: { $regex: '.*' + removeAccents(keyword) + '.*', $options: 'i' } }, { is_delete: false }] }).exec();
    listAgency = [... (await User.find({ $and: [{ role: 'normal' }, { uname: { $regex: '.*' + removeAccents(keyword) + '.*', $options: 'i' } }, { is_delete: false }] }).skip(skip).limit(perPage)
      .sort({
        _id: -1
      }).exec())];
  }
  ...
  if (searchType === 3) {
    if (keyword.length === 24) {
      countAllDocument = await User.countDocuments({ $and: [{ role: 'normal' }, { _id: keyword }, { is_delete: false }] }).exec();
      listAgency = [... (await User.find({ $and: [{ role: 'normal' }, { _id: keyword }, { is_delete: false }] }).skip(skip).limit(perPage)
        .sort({
          _id: -1
        }).exec())];
    } else {
...
exports.getListAdmin = async (req, res) => {
  const listAdmin = await User.find({ $and: [{ role: { $ne: 'normal' } }, { _id: { $ne: req.id_admin } }] }).exec();
...
exports.editTree = async (req, res) => {
  ...
  if (moveItem.parent_id !== process.env.INVITE_CODE) {
    const moveFatherTree = await Tree.findOne({ parent: moveItem.parent_id }).exec();
  ...
  if (moveItem.parent_id !== rootItem._id) {
    ...
    await User.findOneAndUpdate({ _id: moveItem._id }, { parent_id: rootItem._id });
...
exports.deleteUser = async (req, res) => {
  ...
  if (user.parent_id !== process.env.INVITE_CODE) {
    ...
    if (user.group_number === "1") {
      ...
      if (index > -1) {
        newGroup = [...treeParent.group1.slice(0, index), ...treeParent.group1.slice(index + 1)];
        await Tree.findOneAndUpdate({ _id: treeParent._id }, { group1: newGroup }).exec();
      }
    }
    if (user.group_number === "2") {
      const index = treeParent.group2.indexOf(id);
      if (index > -1) {
        newGroup = [...treeParent.group2.slice(0, index), ...treeParent.group2.slice(index + 1)];
        await Tree.findOneAndUpdate({ _id: treeParent._id }, { group2: newGroup }).exec();
      }
    }
    if (user.group_number === "3") {
      const index = treeParent.group3.indexOf(id);
      if (index > -1) {
        newGroup = [...treeParent.group3.slice(0, index), ...treeParent.group3.slice(index + 1)];
        await Tree.findOneAndUpdate({ _id: treeParent._id }, { group3: newGroup }).exec();
      }
    }
  }
  ...
  await User.updateMany({ parent_id: id }, { parent_id: process.env.INVITE_CODE }).exec();
  res.json({
    status: 200,
    message:
      "Đã xóa thành công",
...
const deleteUserAndTree = async (id) => {
  ...
  if (user.buy_package === '3' || user.buy_package === '4') {
    await User.updateMany({ email: user.email }, { is_delete: true, delete_time: new Date(), active: false }).exec();
  } else {
    await User.findOneAndUpdate({ _id: id }, { is_delete: true, delete_time: new Date(), active: false }).exec();
  }
  // await Tree.deleteOne({ parent: id }).exec();
  await Tree.findOneAndUpdate({ parent: id }, { is_delete: true, delete_time: new Date() }).exec();
  await Bonus.findOneAndUpdate({ receive_mem_id: id }, { is_delete: true, delete_time: new Date() }).exec();
  var list = await Transaction.find({ user_id: id }).exec();
  for (var element of list) {
    //await Commission.deleteMany({ trans_id: element._id }).exec();
    await Commission.updateMany({ trans_id: element._id }, { is_delete: true, delete_time: new Date() }).exec();
  }
  //await Transaction.deleteMany({ user_id: id }).exec();
  await Transaction.updateMany({ user_id: id }, { is_delete: true, delete_time: new Date() }).exec();
  //await Commission.deleteMany({ receive_mem_id: id }).exec();
  await Commission.updateMany({ join_mem_id: id }, { is_delete: true, delete_time: new Date() }).exec();
}
...
exports.changeDate = async (req, res) => {
  ...
  for (let user of listUser) {
    ...
    if (created_time) {
      ...
      await User.findOneAndUpdate({ _id: user._id }, { created_time: newTime }).exec();
    }
    if (birthday) {
      let arr1 = birthday.split('/');
      let newTime1 = arr1[2] + '-' + arr1[1] + '-' + arr1[0];
      await User.findOneAndUpdate({ _id: user._id }, { birthday: newTime1 }).exec();
    }
    if (id_time) {
      let arr2 = id_time.split('/');
      let newTime2 = arr2[2] + '-' + arr2[1] + '-' + arr2[0];
      await User.findOneAndUpdate({ _id: user._id }, { id_time: newTime2 }).exec();
    }
    if (expire_time) {
      let arr3 = expire_time.split('/');
      let newTime3 = arr3[2] + '-' + arr3[1] + '-' + arr3[0];
      await User.findOneAndUpdate({ _id: user._id }, { expire_time: newTime3 }).exec();
    }
  }
}
...
exports.renderUser = async (req, res) => {
  ...
  for (let user of listUser) {
    ...
    if (!user.old_parent_id || user.old_parent_id == "") {
      // update parent_id
      await User.findOneAndUpdate({ _id: user._id }, { parent_id: process.env.INVITE_CODE
      ...
      await renderUserFromExcel(user
...
const renderUserFromExcel = async (user
  // , avatar, birthday, id_time, created_time, full_name, expire_time, uname
  ) => {
  const {
    _id,
  ...
  const parent = await User.findOne({ old_id: old_parent_id }).select('_id').exec();
  if (!parent) {
    console.log("PARENT NOT FOUND!!!");
    // update parent_id
    await User.findOneAndUpdate({ _id }, { parent_id: process.env.INVITE_CODE
      // , avatar, birthday, id_time, created_time, full_name, is_clone: false, expired: false, role: 'normal', expire_time, uname, is_partner: true, is_delete: false, active: true
     }).exec();
    return;
  }
  const parentTree = await Tree.findOne({ parent: parent._id }).select('_id group1 group2 group3').exec();
  if (!parentTree) {
  ...
  // update parent_id
  await User.findOneAndUpdate({ _id }, { parent_id: parent._id
    // , avatar, birthday, id_time, created_time, full_name, is_clone: false, expired: false, role: 'normal', expire_time, uname, is_partner: true, is_delete: false, active: true 
  }).exec();
...
exports.deleteOutUser = async (req, res) => {
  const listUser = await User.find({ $and: [{ parent_id: "AMERITECAIPS1109" }, { full_name: '' }] }).exec();
  for (let user of listUser) {
    //await User.deleteOne({ _id: user._id }).exec();
    await User.findOneAndUpdate({ _id: user._id }, { is_delete: true, delete_time: new Date() }).exec();
    //await Tree.deleteOne({ parent: user._id }).exec();
    await Tree.findOneAndUpdate({ parent: user._id }, { is_delete: true, delete_time: new Date() }).exec();
  }
  res.send("deleted");
}
...
exports.updateCreateTime = async (req, res) => {
  ...
  for (let user of listUser) {
    if (user.created_time && user.created_time !== "") {
      ...
      await User.findOneAndUpdate({ _id: user._id }, { created_time: created_time }).exec();
      return;
    }
  }
  res.send("updated");
...
const updateTree = async (group, id, parentTree) => {
  if (group === "1") {
    if (!parentTree.group1.includes(id)) {
      ...
      await Tree.findOneAndUpdate({ _id: parentTree._id }, { group1: [...newArr1] }).exec();
    }
  }
  ...
  if (group === "2") {
    if (!parentTree.group2.includes(id)) {
      ...
      await Tree.findOneAndUpdate({ _id: parentTree._id }, { group2: [...newArr2] }).exec();
    }
  }
  ...
  if (group === "3") {
    if (!parentTree.group3.includes(id)) {
      ...
      await Tree.findOneAndUpdate({ _id: parentTree._id }, { group3: [...newArr3] }).exec();
    }
  }
}
...
exports.updateTree = async (req, res) => {
  ...
  for (let user of listUser) {
    ...
    const groupcon2 = [];
    const groupcon3 = [];
    var listChild = await User.find({ parent_id: user._id, is_delete: false }).exec();
    for (let child of listChild) {
      if (!groupcon1.includes(mongoose.Types.ObjectId(child._id)) || !groupcon2.includes(mongoose.Types.ObjectId(child._id)) || !groupcon3.includes(mongoose.Types.ObjectId(child._id))) {
    ...
    await Tree.findOneAndUpdate({ _id: tree._id }, {
      group1: groupcon1,
      group2: groupcon2,
      group3: groupcon3,
    }).exec()
...
exports.helperInsertCallLevel = async (req, res,) => {
  ...
  for (const element of list) {
    ...
    await User.countDocuments({ parent_id: element._id, buy_package: "3" }, function (err, c) {
      point += c * 9;
    });
    element.point = point;
...
exports.updateGroupNumber = async (req, res) => {
  ...
  for (let user of listUser) {
    if (user.group_number && user.group_number !== "") {
      ...
      await User.findOneAndUpdate({ _id: user._id }, { group_number: '1' }).exec();
    }
  }
}

exports.setIsCloneAcc = async (req, res) => {
  const listUser = await User.find({is_clone: {$ne: false}}).sort({_id: -1}).exec();
  for (let user of listUser) {
    const mapUser = await User.find({$and: [{email: user.email}, {_id: {$ne: user._id}}]}).exec()
    if(mapUser) {
      const splitUserName = user.full_name.split(' ');
      if(splitUserName[splitUserName.length - 1] === '1' || splitUserName[splitUserName.length - 1] === '2' || splitUserName[splitUserName.length - 1] === '3') {
        await User.findOneAndUpdate({ _id: user._id }, { is_clone: true, group_number: '1' }).exec();
      } else if(splitUserName[splitUserName.length - 1] === '4' || splitUserName[splitUserName.length - 1] === '5' || splitUserName[splitUserName.length - 1] === '6') {
        await User.findOneAndUpdate({ _id: user._id }, { is_clone: true, group_number: '2' }).exec();
      } else if(splitUserName[splitUserName.length - 1] === '7' || splitUserName[splitUserName.length - 1] === '8' || splitUserName[splitUserName.length - 1] === '9') {
        await User.findOneAndUpdate({ _id: user._id }, { is_clone: true, group_number: '3' }).exec();
      } else {
        await User.findOneAndUpdate({ _id: user._id }, { is_clone: false, group_number: '1' }).exec();
      }
    } else {
      await User.findOneAndUpdate({ _id: user._id }, { is_clone: false, group_number: '1' }).exec();
    }
...
exports.updatePassword = async (req, res) => {
  const listUser = await User.find().exec();
  for (let user of listUser) {
    await User.findOneAndUpdate({ _id: user._id }, { password: "$2b$10$Gq3FDe6JPJN30GQ/R6qfdeYsUg7TY0.ZMsyBjHLJgYTFxcV7d7wra" }).exec();
  }
}
...
exports.updateRole = async (req, res) => {
  const listUser = await User.find().exec();
  for (let user of listUser) {
    await User.findOneAndUpdate({ _id: user._id }, { is_partner: true ,  is_delete: false ,  expired: false ,  active: true }).exec();
  }
  res.send("ok");
}
...
exports.createCloneAcc = async (req, res) => {
  ...
  const listUser = await User.find({ $or: [{ buy_package: "4" }, { buy_package: "3" }] }).exec();
  for (let user of listUser) {
    if (user.buy_package === '3') {
  ...
  for (let user of list) {
    if (user.child_id && user.child_id !== "") {
      ...
      if (parentId !== process.env.INVITE_CODE) {
        ...
        const newParent = await User.findOne({ $and: [{ full_name: parent.full_name + ' ' + user.child_id }, { parent_id: parentId }] }).exec();
        if (newParent) {
          const newParentTree = await Tree.findOne({ parent: newParent._id }).exec();

          if (user.group_number === "1") {
            if (!newParentTree.group1.includes(user._id)) {
              var newArr1 = [...newParentTree.group1, user._id];
              await Tree.findOneAndUpdate({ _id: newParentTree._id }, { group1: [...newArr1] }).exec();
            }
          }
          ...
          if (user.group_number === "2") {
            if (!newParentTree.group2.includes(user._id)) {
              ...
              await Tree.findOneAndUpdate({ _id: newParentTree._id }, { group2: [...newArr2] }).exec();
            }
          }
          ...
          if (user.group_number === "3") {
            if (!newParentTree.group3.includes(user._id)) {
              ...
              await Tree.findOneAndUpdate({ _id: newParentTree._id }, { group3: [...newArr3] }).exec();
            }
          }

          await User.findOneAndUpdate({ _id: user._id }, { parent_id: newParent._id }).exec();
        }
      }
    }
  }
...
exports.updateCloneToBuyPackage3 = async (req, res) => {
  ...
  for (let user of list) {
    ...
    await Tree.findOneAndUpdate({ parent: user._id }, { buy_package: "3" }).exec();
  }
...
exports.deleteUserIsClone = async (req, res) => {
  ...
  for (let user of listUser) {
    let groupNumber = user.full_name.split(" ")[user.full_name.split(" ").length - 1];
    var treeParent = await Tree.findOne({ parent: user.parent_id }).exec();
...
exports.updateLevelUpdateEmpty = async (req, res) => {
  const list = await User.find({ role: { $ne: 'admin' } }).exec();
  for (let user of list) {
    await User.findOneAndUpdate({ _id: user._id }, { level_up_time: "" }).exec();
  }
}
...
exports.updateAccountType = async (req, res) => {
  ...
  for (let user of list) {
    if (!user.ss) {
      await User.findOneAndUpdate({ _id: user._id }, { account_type: 'vi' }).exec();
    } else {
      await User.findOneAndUpdate({ _id: user._id }, { account_type: 'en' }).exec();
    }
  }
}
...
exports.convertDateToISO = async (req, res) => {
  ...
  for (let user of list) {
    if (user.created_time) {
      await User.findOneAndUpdate({ _id: user._id }, { created_time: new Date(user.created_time).toISOString() }).exec();
    }
    if (user.birthday) {
      await User.findOneAndUpdate({ _id: user._id }, { birthday: new Date(user.birthday).toISOString() }).exec();
    }
    if (user.id_time) {
      await User.findOneAndUpdate({ _id: user._id }, { id_time: new Date(user.id_time).toISOString() }).exec();
    }
    if (user.expire_time) {
      await User.findOneAndUpdate({ _id: user._id }, { expire_time: new Date(user.expire_time).toISOString() }).exec();
    }
    if (user.level_up_time) {
      await User.findOneAndUpdate({ _id: user._id }, { level_up_time: new Date(user.level_up_time).toISOString() }).exec();
    }
  }
  res.send("ok");
}
...
exports.test = async (req, res) => {
  ...
  const listUser = await User.find({ expired: false, role: 'normal' }).sort({ _id: -1 }).exec();
  ...
  const listRemind1User = listUser.filter(user => {
    return moment(user.expire_time).diff(moment(date_current), "days") == 15 && user.is_clone == false;
  });
  const listRemind2User = listUser.filter(user => {
    return moment(user.expire_time).diff(moment(date_current), "days") == 0 && user.is_clone == false;
  });
  res.json({
    date: new Date(date),
    listRemind1User,
...
exports.getListDeleteUser = async (req, res) => {
  ...
  var list = await User.find({ $and: [{ is_delete: true }, { uname: { $regex: '.*' + removeAccents(keyword) + '.*', $options: 'i' } }] }).limit(perPage).skip(skip).sort({ _id: -1 }).exec();
  res.json({
    status: 200,
    data: {
      list,
...
exports.renewLicense = async (req, res) => {
  ...
  console.log(req.body);
  var user = await User.findOne({ _id: id }).exec();
  var trans_check = await Transaction.findOne({ user_id: id, status: "pending", is_renew: true }).sort({ created_time: -1 }).exec();
  if (trans_check) {
    return res.json({
      status: 404,
      data: "",
  ...
  var oldTrans = await Transaction.findOne({ user_id: id }).sort({ created_time: -1 }).exec();
  var oldComm = await Commission.findOne({ $and: [{ join_mem_id: id }, { is_delete: false }] }).sort({ created_time: -1 }).exec();

  var date = new Date(user.expire_time);
  var expire_date = new Date(date.getFullYear() + 1, date.getMonth(), date.getDate(), 0, 0, 0).toISOString();
  ...
  if (oldComm) {
    ...
    await trans.save();
    let receive_mem = await User.findOne({ _id: oldComm.receive_mem_id }).exec();
    ...
    let parent = await User.findOne({ $and: [{ _id: user.parent_id }, { is_delete: false }] }).exec();
    if (parent) {
      let trans = new Transaction({
        status: "success",
        created_time: new Date(),
...
exports.checkExpired = async (req, res) => {
  ...
  res.json({
    ...
    message: "listRemind1User " + listRemind1User.length + " | listRemind2User " + listRemind2User.length + " | Expired " + listExpiredUser.length,
    errors: [],
  });
}
...
exports.moveToChildOfParent = async (req, res) => {
  ...
  for (let user of list) {
    if (user.child_id && user.child_id !== "") {
      ...
      const newParent = await User.findOne({ full_name: parent.full_name + ' ' + user.child_id }).exec();
...
exports.changeAvatar = async (req, res) => {
  ...
  for (let user of list) {
    ...
    await User.findOneAndUpdate({ _id: user._id }, { avatar: newAvatar }).exec();
  }
...
exports.updateCommissionToSuccess = async (req,res) => {
  ...
  for(let com of listCom) {
    await Commission.findOneAndUpdate({ _id: com._id },
      {
          status: "success",
